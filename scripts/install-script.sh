...
$install $verbose --owner=$fvuser --group=$fvgroup --mode=644 $scriptd/derby.properties $root$prefix/etc/flowvisor/derby.properties
JAVA=`which java`
CHOWN=`which chown`
...
$install $verbose --owner=$fvuser --group=$fvgroup --mode=644 $scriptd/fvlog.config $root$prefix/etc/flowvisor/fvlog.config

echo Installing documentation
cd $owd
$install $verbose --owner=$binuser --group=$bingroup --mode=644 $DOCS $root$prefix/share/doc/flowvisor

if [ ! -f $root$prefix/etc/flowvisor/config.json ] ; then 
    echo Generating a default config FlowVisor config
    install_root=$root $root$prefix/sbin/fvconfig generate $root$prefix/etc/flowvisor/config.json
    $CHOWN -R $fvuser:$fvgroup $root$prefix/share/db/flowvisor
    $CHOWN -R $fvuser:$fvgroup $root$prefix/etc/flowvisor/config.json
else
